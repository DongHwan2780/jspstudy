...
public class BoardDaoImpl implements BoardDao {
  ...
  private BoardDaoImpl() {
    // META-INF\context.xml 파일에 명시된 Resource 를 이용해 DataSource 객체 생성
    try {
      Context context = new InitialContext();
      Context env = (Context)context.lookup("java:comp/env");
      dataSource = (DataSource)env.lookup("jdbc/myoracle");
    } catch(NamingException e) {
      System.out.println("관련 자원 찾을 수 없음");
    }
  }
  ...
  public int insertBoard(BoardDto board) {
    ...
    try {
      ...
      String sql = "INSERT INTO BOARD_T(BOARD_NO, TITLE, CONTENTS, MODIFIED_AT, CREATED_AT) "
                  + "VALUES(BOARD_SEQ.NEXTVAL, ?,?, CURRENT_DATE ,CURRENT_DATE)";
  ...
  public List<BoardDto> selectBoardList(Map<String, Object> map) {
    ...
    try {
      con = dataSource.getConnection();
      String sql = "SELECT BOARD_NO, TITLE, CONTENTS, MODIFIED_AT, CREATED_AT FROM BOARD_T "
