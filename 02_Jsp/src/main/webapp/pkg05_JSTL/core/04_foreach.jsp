...
<body>
  <%-- 
   ...
   <%-- 향상 for 문 (리스트) --%>
    <% pageContext.setAttribute("hobbies", Arrays.asList("trabel", "cooking", "game")); %>
   <c:forEach var="hobby" items="${hobbies}" varStatus="vs">
    <div>${hobby} / ${vs.index } / ${vs.count }</div>
   </c:forEach>  
   
   <%
   List<Post> posts = Arrays.asList(new Post("제목1","기자1","작성일1"), new Post("제목2","기자2","작성일2"), new Post("제목3","기자3","작성일3"));
   pageContext.setAttribute("posts", posts);
   %>
   <c:forEach var="post" items="${posts}" varStatus="vs">
    <div>${post.title} / ${post.reporter} / ${post.createdAt }/ ${vs.index } / ${vs.count }</div>
   </c:forEach>
