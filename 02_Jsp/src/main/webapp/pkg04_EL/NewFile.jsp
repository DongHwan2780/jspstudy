...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <%-- 
      ...
      1. pageContext    : this(NewFile_jsp)       : 현재 JSP인 NewFile.jsp에서만 접근 가능
...
<body>
  ...
  %>
  <div>${book.title}</div>        <%-- book.getTitle()방식으로 호출 --%>
  <div>${book.author}</div>       <%-- book.getauthor()방식으로 호출 --%>
  <div>${book.price}</div>        <%-- book.getprice()방식으로 호출 --%>
  
  <hr>
  <%-- JSP Bind 영역에 저장된 Map 정보 확인 --%>
  <%
  Map<String, Object> map = Map.of("title", "어린왕자", "author", "생택쥐페리", "price" ,"10000");
  ...
  <hr>
  <%-- List에 Book 객체를 3개 저장하고 EL로 확인하기 --%>
  ...
  for(int i = 1; i <= 3; i++)
  {
    books.add(Book.builder().title("제목" + i).author("저자" + i).price(i * 1000).build());
  }
  ...
  <%
    List<Book> bookss = Arrays.asList(
    ...
    pageContext.setAttribute("books", books);
  %>
  <div>${bookss.get(0).title}</div>
  <div>${bookss.get(0).author}</div>
  <div>${bookss.get(0).price}</div>
  <div>${bookss[1].title}</div>
  <div>${bookss[1].author}</div>
  <div>${bookss[1].price}</div>
